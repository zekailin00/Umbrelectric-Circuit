...
const int switch3Port = 9; // stepper
...
void loop() {
  ...
  if(read((double)analogRead(switch2Port)) && EEPROM.read(1) == 0) { // if switch 2 is powered, and the clamp is closed
    myservo.write(0); // TODO: figure out if this is the correct angle
    EEPROM.write(1, 1); // set the clamp state to open
  }
  if(read((double)analogRead(switch2Port)) == 0 && EEPROM.read(1) == 1) { // if switch 2 is not powered and the clamp is open
    myservo.write(90); // TODO: figure out if this is the correct angle
    EEPROM.write(1, 0); // set the clamp state to closed
  }

  if(read((double)analogRead(switch3Port)) && EEPROM.read(3) <= revolutionsToOpen) { // if switch 3 is powered and we still have revolutions to go
  ...
  if(read((double)analogRead(switch3Port)) == 0 && (EEPROM.read(2) > 0 || EEPROM.read(3) > 0)) { // if switch 3 is not powered and we are not in the closed state
    int stepsTaken = EEPROM.read(2) + EEPROM.read(3) * stepsPerRevolution; // number of steps we have taken to open the umbrella fully
